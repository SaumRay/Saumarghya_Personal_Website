...
export const uploadGalleryImages = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const newImages = files.map((file, i) => ({ key: file.key, url: file.location, caption: captions[i] || "", uploadedAt: new Date() }));
    const gallery = await Gallery.findByIdAndUpdate(req.params.id, { $push: { images: { $each: newImages } } }, { new: true });
    if (!gallery) { res.status(404).json({ success: false, message: "Gallery not found" }); return; }
