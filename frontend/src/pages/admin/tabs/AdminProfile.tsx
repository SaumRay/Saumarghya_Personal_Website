...
interface Profile { name: string; tagline: string; bio: string; email: string; phone: string[]; location: string; linkedin: string; github: string; instagram: string; profileImages: ProfileImage[]; resumeUrl: string; }
...
export function AdminProfile() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/profile`, { headers })
      .then((r) => r.json())
      .then((d) => { if (d.data) { setProfile(d.data); setForm(d.data); } });
  }, []);
    ...
    <div className="max-w-2xl space-y-8">
      ...
      <div className="glass-card rounded-2xl p-6 border border-white/10">
        ...
        <div className="mt-4">
          ...
          <input value={(form.tagline as string) || ""} onChange={(e) => setForm({ ...form, tagline: e.target.value })}
            className="w-full px-4 py-2.5 rounded-xl bg-foreground/5 border border-foreground/10 text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/40" placeholder="Your tagline..." />
        </div>
        <div className="mt-4">
          <label className="text-xs font-medium text-foreground/60 mb-1 block">Bio</label>
          <textarea rows={4} value={(form.bio as string) || ""} onChange={(e) => setForm({ ...form, bio: e.target.value })}
            className="w-full px-4 py-2.5 rounded-xl bg-foreground/5 border border-foreground/10 text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 resize-none" placeholder="Your bio..." />
        </div>
        <button onClick={handleSave} disabled={saving} className="mt-5 flex items-center gap-2 px-5 py-2.5 rounded-xl bg-primary/20 text-primary border border-primary/20 hover:bg-primary/30 text-sm font-medium disabled:opacity-50 transition-all">
      ...
      <div className="glass-card rounded-2xl p-6 border border-white/10">
        ...
        <div className="flex gap-3 mb-5 flex-wrap">
          <select value={imgLabel} onChange={(e) => setImgLabel(e.target.value)}
          ...
          {imgFile && (
            <button onClick={uploadImage} disabled={uploading} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-primary/20 text-primary border border-primary/20 text-sm font-medium disabled:opacity-50 hover:bg-primary/30 transition-all">
